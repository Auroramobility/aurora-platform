...
Defines the ownership relationship and lifecycle state. Financial terms are stored separately in `financing_terms`.
...
Financial terms are not duplicated on this table. See `financing_terms`, `payment_schedule`, `payments`, and `payment_allocations` for the financial domain.
...
- `financing_terms`: contractual financing inputs and agreed payment terms for one ownership plan.
- `payment_schedule`: individual amounts due and their payment state.
- `payments`: immutable/trusted transaction records, optionally linked to a schedule item and external provider transaction.

Customers have read-only access to their own financing terms and schedule. Payment writes remain trusted server-side operations.
